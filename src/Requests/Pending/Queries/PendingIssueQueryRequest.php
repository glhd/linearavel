...
use Glhd\Linearavel\Responses\Queries\IssueQueryResponse;
use Glhd\Linearavel\Support\GraphQueryBuilder;

class PendingIssueQueryRequest extends PendingLinearRequest
{
	...
	{
		parent::__construct($connector, GraphQueryBuilder::make('query', 'issue', $args, static::ARGUMENT_TYPES));
	}
	...
	public function response(string ...$fields): IssueQueryResponse
	{
		$query = $this->query->withFields($this->normalizeFields($fields));
		
		$response = $this->connector->send(new LinearRequest(IssueQueryResponse::class, $query))->throw();
		
		assert($response instanceof IssueQueryResponse);
