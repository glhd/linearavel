...
{
	protected const DEFAULT_ATTRIBUTES = ['id', 'createdAt', 'updatedAt', 'email', 'role', 'external', 'archivedAt', 'acceptedAt', 'expiresAt', 'metadata'];

	protected const ARGUMENT_TYPES = ['id' => 'String!'];

	public function __construct(LinearConnector $connector, public array $args = [])
	{
		parent::__construct($connector, GraphQueryBuilder::make('query', 'organizationInvite', $args, static::ARGUMENT_TYPES));
	}
	...
	public function response(string ...$fields): OrganizationInviteQueryResponse
	{
		...
		$response = $this->connector->send(new LinearRequest(OrganizationInviteQueryResponse::class, $query))->throw();
