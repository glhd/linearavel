...
use Glhd\Linearavel\Responses\Queries\TeamsQueryResponse;
use Glhd\Linearavel\Support\GraphQueryBuilder;

class PendingTeamsQueryRequest extends PendingLinearRequest
{
	...
	{
		parent::__construct($connector, GraphQueryBuilder::make('query', 'teams', $args, static::ARGUMENT_TYPES));
	}
	...
	public function response(string ...$fields): TeamsQueryResponse
	{
		$query = $this->query->withFields($this->normalizeFields($fields));
		
		$response = $this->connector->send(new LinearRequest(TeamsQueryResponse::class, $query))->throw();
		
		assert($response instanceof TeamsQueryResponse);
